...
# STATES OF THE APP
LOGIN, NEW_USER, CHOOSING, INTR_MEDICINE, INTR_RECEIPT, CHECK_REC, GET_CN, CHECK_REM, JOURNEY, END = range(10)

# FUNCTIONS FOR COMMUNICATING WITH DATA BASE
QUERIES = ['CHECK USER', 'CHECK PASSWORD', 'NEW PASSWORD', 'INTRODUCE RECEIPT', 'TASKS CALENDAR', 'CURRENT TREATMENT', 'JOURNEY',
           'GET REMINDER', 'DELETE REMINDER']
# MANAGE WHOLE INFORMATION
aide_bot = {}

# TAGS TO MANAGE INTRODUCING MEDICINES
INTR_RECEIPT_MSSGS = ["What is the medicine's name (CN)?", "How many pills are in the packaging?",
...
reply_keyboard = [['Introduce Receipt', 'Calendar'], ['Introduce Medicine', 'History'], ['Current treatments', 'Journey'], ['Inventory', 'Delete reminder'], ['Show information', 'Exit']]
...
class PillDora:
    ...
    def start(self, update, context):
        ...
        aide_bot[user_id] = {'states': [LOGIN, LOGIN], 'intr_receipt_counter': 0,
    ...
    def manage_response(self, update, context):
        ...
        if (self.get_query(user_id) != "None") and (self.get_query(user_id) != {"None": "None"}):
            ...
            if response['function'] == 'INTRODUCE RECEIPT':
            ...
            if response['function'] == "DELETE REMINDER":
                if response['parameters']:
                    ...
                    update.message.reply_text("Medicine introduced did not exist in your current medicines taken.")
    ...
    def intr_receipt(self, update, context):
        logger.info('User introducing new medicine')
        update.message.reply_text(INTR_RECEIPT_MSSGS[self.get_counter(update.message.from_user.id)])
        return self.set_state(update.message.from_user.id, INTR_RECEIPT)

    
    # Creates a query of the medicine and changes de state to CHECKING if the medicine inserted is correct or not
    def send_new_receipt(self, update, context):
        try:
            user_id = update.message.from_user.id
        ...
        if self.get_counter(user_id) != len(INTR_RECEIPT_MSSGS):
            if self.get_counter(user_id) < 3:
                update.message.reply_text(INTR_RECEIPT_MSSGS[self.get_counter(user_id)])
                return INTR_RECEIPT
            else:
                context.bot.send_message(chat_id=user_id,
                                         text=INTR_RECEIPT_MSSGS[self.get_counter(user_id)],
            ...
            self.set_function(user_id, 'INTRODUCE RECEIPT')
            return self.set_state(user_id, CHECK_REC)
        ...
        if selected:
            ...
            if self.get_states(user_id)[0] == CHOOSING:
                ...
                if self.get_states(user_id)[1] == CHOOSING:
                    ...
                    self.set_state(user_id, JOURNEY)
            elif self.get_states(user_id)[0] == INTR_RECEIPT:
                context.bot.send_message(chat_id=user_id,
                                         text=date.strftime("%Y-%m-%d"),
                                         reply_markup=ReplyKeyboardRemove())
                self.set_medicine(user_id, self.get_counter(user_id), date.strftime("%Y-%m-%d"))
                self.send_new_receipt(update, context)
    ...
    def main(self):
        ...
        conv_handler = ConversationHandler(
            ...
            states={
                ...
                CHOOSING: [MessageHandler(Filters.regex('^Introduce Receipt'),
                                          self.intr_receipt),
                           MessageHandler(Filters.regex('^Calendar$'),
                                          self.see_calendar),
                           MessageHandler(Filters.regex('^Current Treatments'),
                ...
                INTR_RECEIPT: [MessageHandler(Filters.text, self.send_new_receipt)],
                CHECK_REC: [MessageHandler(Filters.regex('^YES$'), self.manage_response),
                            MessageHandler(Filters.regex('^NO$'), self.intr_receipt)
                            ],
